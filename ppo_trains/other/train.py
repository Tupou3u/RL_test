...
if __name__ == '__main__':
    ...
    for epoch in range(1, int(1e10)):
            ...
            while not (terminated or truncated):
                with torch.no_grad():
                    action, value, logprob, _ = rollout_model.get_action_and_value(
                        torch.tensor(state).to(ROLLOUT_DEVICE).unsqueeze(0)
                    )
        ...
        agent.train(
            torch.tensor(states[:ROLLOUT_LEN-1], dtype=torch.float32).to(TRAIN_DEVICE), 
            torch.tensor(actions[:ROLLOUT_LEN-1], dtype=torch.float32).to(TRAIN_DEVICE), 
