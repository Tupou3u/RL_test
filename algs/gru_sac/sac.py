...
class SACAgentGRU:
    ...
    def train_step(self, replayBuffer: ReplayBuffer):    
        ...
        next_state_log_pi = torch.cat([all_log_pi[inds, row] for row, inds in enumerate(next_state_inds)]).reshape(seq_len, -1)
        next_state_action_probs = torch.cat([all_action_probs[inds, row] for row, inds in enumerate(next_state_inds)]).reshape(seq_len, -1)
        ...
        with torch.no_grad():
            ...
            qf1_next_target = torch.cat([all_qf1_target[inds, row] for row, inds in enumerate(next_state_inds)]).reshape(seq_len, -1)
            qf2_next_target = torch.cat([all_qf2_target[inds, row] for row, inds in enumerate(next_state_inds)]).reshape(seq_len, -1)
        ...
        state_inds = [torch.where(d[:l-1] == 0)[0] for d, l in zip(all_dones, lenghts)] 
        qf1_values = torch.cat([all_qf1_values[inds, row] for row, inds in enumerate(state_inds)]).reshape(seq_len, -1)
        qf2_values = torch.cat([all_qf2_values[inds, row] for row, inds in enumerate(state_inds)]).reshape(seq_len, -1)
        ...
        # ACTOR training
        action_probs =  torch.cat([all_action_probs[inds, row] for row, inds in enumerate(state_inds)]).reshape(seq_len, -1)
        log_pi =  torch.cat([all_log_pi[inds, row] for row, inds in enumerate(state_inds)]).reshape(seq_len, -1)
