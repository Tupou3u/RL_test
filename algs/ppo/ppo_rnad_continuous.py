...
class RNADNetwork(nn.Module):
    def __init__(self, input_dim):
        ...
        self.fc = nn.Sequential(
            layer_init(nn.Linear(input_dim, LAYER_SIZE)),
            nn.ReLU(),
            layer_init(nn.Linear(LAYER_SIZE, LAYER_SIZE)),
            nn.ReLU(),
            layer_init(nn.Linear(LAYER_SIZE, LAYER_SIZE))
...
class PPO_MLP:
    def __init__(self, 
        ...
        self.rnad_beta = 0.1
        self.rnad_target = RNADNetwork(state_dim).to(self.device)
        self.rnad_predictor = RNADNetwork(state_dim).to(self.device)
        self.rnad_predictor_optimizer = optim.Adam(self.rnad_predictor.parameters(), lr=learning_rate, eps=1e-5)
        self.num_trains = 0
    ...
    def _rnad(self, states, rewards):
        with torch.no_grad():
            target_output = self.rnad_target(states)

        predictor_output = self.rnad_predictor(states)
        loss = torch.mean((target_output - predictor_output) ** 2, dim=-1)
        total_rewards = rewards + self.rnad_beta * loss.detach()
        self.rnad_predictor_optimizer.zero_grad()
        loss.mean().backward()
        self.rnad_predictor_optimizer.step()
        return total_rewards, loss.mean().item()

    def train(self, states, actions, rewards, dones, logprobs, values):
        total_rewards, rnad_loss = self._rnad(states, rewards)
        adv_gae = self.__get_adv_gae(total_rewards, values, dones)
        y = adv_gae + values[:-1]
        ...
        if self.log_dir:
            ...
            self.writer.add_scalar("rnad/loss", rnad_loss, self.num_trains)
        self.num_trains += 1
